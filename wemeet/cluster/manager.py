...
def cleanup_zombie_containers():
    ...
    if state.DOCKER_CLIENT is None:
        _obslog.log_event("[Docker SDK] 도커 데몬 미연결로 잔존 컨테이너 정리 작업을 스킵합니다.")
        return
        
    _obslog.log_event("[Docker SDK] 기존 잔존 동적 컨테이너 청소 작업을 시작합니다...")
    try:
        ...
        if not targets:
            _obslog.log_event("[Docker SDK] 정리할 잔존 컨테이너가 없습니다.")
            return
        ...
        def remove_container(c):
            try:
                _obslog.log_event(f"[Docker SDK] 잔존 컨테이너 강제 제거 시작: {c.name}")
                c.remove(force=True)
                _obslog.log_event(f"[Docker SDK] 잔존 컨테이너 강제 제거 성공: {c.name}")
            except Exception as e:
                _obslog.log_event(f"[Docker SDK 에러] 컨테이너 {c.name} 제거 실패: {e}")
        ...
        _obslog.log_event(f"[Docker SDK] 총 {len(targets)}개의 잔존 컨테이너에 대해 강제 정리 명령을 병렬 전송했습니다.")
        
    except Exception as e:
        _obslog.log_event(f"[Docker SDK 에러] 잔존 컨테이너 조회 중 에러 발생: {e}")


def _load_node_config(node_type):
    """노드 스펙(cpu/mem 한도·GPU 스케일)을 env_config(cost_model.yaml 유일 진실)에서 로드합니다.

    Args:
        node_type (str): 노드 유형("spot_a"/"spot_b" 등).

    Returns:
        tuple[float, int, float]: (cpu_limit, mem_limit_mb, gpu_scale_factor).
    """
    # 폴백 기본값(키 누락 시)
...
def scale_out_worker(node_type):
    ...
    if FailureSimulator.check_out_of_capacity(node_type):
        _obslog.log_event(f"[Docker SDK] OutOfCapacity 감지: Spot-{node_type[-1].upper()} 자원 공급 부족으로 인해 노드 증설이 거절되었습니다.")
        return False
    ...
    try:
        ...
        # 5. 실행 커맨드 구성
        cmd = [
            "python", "-m", "wemeet.transport.worker",
            "--id", worker_id,
            "--type", node_type,
        ...
        _obslog.log_event(f"[Docker SDK] 신규 Spot 컨테이너 가동 완료: ID='{worker_id}' | Name='{container_name}' | Port={candidate_port}")
        return True

    except Exception as e:
        _obslog.log_event(f"[Docker SDK 에러] 동적 스케일아웃 실행 실패: {e}")
        return False
...
def scale_in_specific_worker(node_type):
    ...
    try:
        ...
        if target_worker_id is None:
            _obslog.log_event(f"[Docker SDK] 감축 경고: 회수 가능한 IDLE 상태의 {node_type} 워커가 존재하지 않습니다.")
            return False
        ...
        try:
            ...
            _obslog.log_event(f"[Docker SDK] IDLE 컨테이너 회수 시작: {target_worker_id} (컨테이너 ID: {container_ref})")
            container.stop(timeout=5)
            container.remove()
            _obslog.log_event(f"[Docker SDK] IDLE 컨테이너 회수 성공: {target_worker_id}")
        except Exception as e:
            _obslog.log_event(f"[Docker SDK 경고] 컨테이너 직접 조작 실패 ({e}), GCS 레지스트리만 소거 처리 진행.")
        ...
        _obslog.log_event(f"[Docker SDK 에러] 스케일인 수행 실패: {e}")

def reclaim_corrupted_worker(worker_id):
    """
    [OOM-Kill Docker SDK 강제 정지 API]
    OOM 발생 워커 노드 컨테이너를 Docker SDK API를 호출해 즉시 강제 정지(stop) 및 삭제(remove)합니다.
    """
    if state.DOCKER_CLIENT is None:
        return False

    container_ref = f"babyray-{worker_id}"
    try:
        container = state.DOCKER_CLIENT.containers.get(container_ref)
        _obslog.log_event(f"[OOM-Kill Daemon] OOM 발생 컨테이너 강제 회수 시작: {worker_id} ({container_ref})")
        # 1초 타임아웃으로 강제 즉시 중단 후 소거
        container.stop(timeout=1)
        container.remove(force=True)
        _obslog.log_event(f"[OOM-Kill Daemon] OOM 발생 컨테이너 강제 회수 완료: {worker_id}")
        return True
    except Exception as e:
        _obslog.log_event(f"[OOM-Kill Daemon 에러] 컨테이너 강제 회수 실패: {e}")
        return False
...
def start_spot_eviction_loop():
    ...
    """
    def eviction_loop():
        _obslog.log_event("=== [Eviction Daemon] 실시간 스팟 강제 회수 모니터링 데몬 기동 ===")
        while True:
            time.sleep(10.0)  # 10초 주기로 회수 여부 심사
            
            # 아키텍처 개선: 메모리 가드 초과로 증설이 막힌 경우, 기존 노드 보존을 위해 강제 회수 동결(Freeze)
            if not is_host_resource_sufficient():
                _obslog.log_event("[Eviction Daemon] 호스트 메모리 부족 감지 -> 기존 Spot 워커 보호를 위해 강제 회수를 일시 중단(Freeze)합니다.")
                continue
                
            # 회수 기본확률의 유일 진실 = cost_model.yaml preemption_probability (env_config)
            preemption_probs = _ec.preemption_probs()
            ...
            for wid, n_type in spot_workers:
                ...
                if FailureSimulator.check_eviction(n_type, p_spot, preemption_probs):
                    ...
                    _obslog.log_event(f"[Eviction Daemon] !!! 스팟 강제 회수(Eviction) 발생 !!! -> 대상: {wid} (확률: {eviction_prob*100:.1f}%)")
                    ...
                    try:
                        if state.DOCKER_CLIENT is not None:
                            ...
                            container.stop(timeout=1)
                            container.remove(force=True)
                            _obslog.log_event(f"[Eviction Daemon] 컨테이너 강제 회수 완료: {container_ref}")
                    except Exception as e:
                        _obslog.log_event(f"[Eviction Daemon 오류] 컨테이너 소거 중 실패: {e}")
