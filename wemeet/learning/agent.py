...
# 보상 가중치·하이퍼파라미터 기본값의 유일 진실: sim_env.yaml (env_config). 생성자 인자로 오버라이드 가능.
_RW = _ec.reward()
_QL = _ec.qlearning()
...
class QLearningAgent:
    ...
    def __init__(self, cost_model_path=None, q_table_path="q_table.json",
                 alpha=_QL["alpha"], gamma=_QL["gamma"], epsilon=_QL["epsilon"],
                 epsilon_min=_QL["epsilon_min"], decay_rate=_QL["decay_rate"]):
        """
        ...
        if q_table_path == "q_table.json":
            # 공유 데이터 디렉토리(data/q_table.json). 경로 계산은 paths 해석기로 일원화.
            from wemeet.config import paths
            self.q_table_path = paths.q_table_path()
        ...
        # 보상 가중치 값의 유일 진실은 sim_env.yaml reward 섹션(_RW). 아래는 그 값을 담을 뿐이다.
        self.SUCCESS_REWARD = _RW["success"]
        self.COST_WEIGHT = _RW["cost_weight"]   # 시간 환산 비용이 매우 작으므로 감점 체감을 위해 대폭 상향(1000.0)
        self.DELAY_PENALTY_WEIGHT = _RW["delay_penalty_weight"]
        # 회수(Eviction)로 태스크가 죽었을 때 부과하는 강한 페널티. 성공 보상을 상회하여
        # "느린 노드에 무거운 작업을 넣으면 회수 룰렛에 여러 번 노출되어 죽는다"를 학습하게 만든다.
        self.EVICTION_PENALTY = _RW["eviction_penalty"]
        # OOM(Out-Of-Memory) 발생 시 부과하는 강력한 페널티. 실제 서비스에서 치명적인 장애이므로 성공 보상을 지우고 감점 유도.
        self.OOM_PENALTY = _RW["oom_penalty"]
        # makespan(체류/실행 시간) 상시 감점 가중치. 데드라인 초과 여부와 무관하게 '느림' 자체에 대가를 부과하여
        # "싸지만 느린" Spot-B가 공짜로 보이는 편향을 제거한다.
        self.MAKESPAN_WEIGHT = _RW["makespan_weight"]
    ...
    def calculate_reward(self, success, execution_time, worker_type, delay_time, deadline_exceeded, current_model=None, co_scheduled_models=None, evicted=False, oom=False):
        """
        ...
        # 3-2. OOM 페널티: 메모리 부족(OOM)으로 태스크가 실패하면 강력한 벌점 부과.
        #      실제 프로덕션 환경의 OOM 장애 치명도를 모사하여, 자원 사양에 맞지 않는 배정을 학습으로 차단.
        if oom:
            reward -= self.OOM_PENALTY
        ...
        if current_model and co_scheduled_models:
            ...
            for other_model in co_scheduled_models:
                ...
                if is_current_mem_bound != is_other_mem_bound:
                    reward += _RW["cosched_bonus"]
                else:
                    # 동일 리소스 성격이 편중되어 자원 경합을 유발한 경우 페널티 감점
                    reward -= _RW["cosched_penalty"]
