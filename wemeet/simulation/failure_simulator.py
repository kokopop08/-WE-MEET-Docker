...
# 확률 수치의 유일 진실은 common/sim_env.yaml + common/cost_model.yaml (env_config 로더).
# 아래 클래스 상수는 그 값을 로드해 담을 뿐이며, 파일/키 부재 시 env_config 내장 폴백으로 동일 값을 유지한다.
_FAIL = _ec.failure()
_OOM = _FAIL["oom"]
...
class FailureSimulator:
    ...
    # --- OOM (Out-of-Memory) 시뮬레이션 설정 (값 출처: sim_env.yaml failure.oom) ---
    OOM_PROB_LSTM = _OOM["legacy_lstm_prob"]  # (레거시) node_type 미지정 호출 시 LSTM 자연 OOM 확률 (8%)
    ...
    OOM_BASE_PROB = _OOM["base_prob"]
    OOM_COLOCATION_STEP = _OOM["colocation_step"]  # 동거 중인 메모리바운드 태스크 압력 가산값 (10%)
    OOM_PROB_CAP = _OOM["prob_cap"]                # OOM 확률 상한 (90%)

    # --- Eviction (Preemption) 시뮬레이션 설정 ---
    # 회수 기본확률의 유일 진실은 cost_model.yaml의 preemption_probability(env_config.preemption_probs()).
    # 별도 폴백 상수를 두지 않고 로더에서 직접 읽어 표시값-실제값 불일치를 원천 차단한다.
    EVICTION_BASE_PROB = _ec.preemption_probs()
    EVICTION_IDLE_FACTOR = _FAIL["eviction"]["idle_factor"]  # 평시(P_spot=0)에는 위험구간 확률의 25%로 감소

    # --- OutOfCapacity 시뮬레이션 설정 ---
    OUT_OF_CAPACITY_PROB = _FAIL["out_of_capacity_prob"]  # Spot 신규 증설 요청 시 공급 부족 거절 확률 (30%)

    # --- 회수 위험구간(danger phase) 사이클 (sim_env.yaml failure.danger_phase) ---
    DANGER_CYCLE_SEC = _FAIL["danger_phase"]["cycle_sec"]    # 30초 사이클
    DANGER_WINDOW_SEC = _FAIL["danger_phase"]["window_sec"]  # 앞 10초가 위험구간
    ...
    def current_danger_phase(now: float = None) -> int:
        ...
        return 1 if (t % FailureSimulator.DANGER_CYCLE_SEC) < FailureSimulator.DANGER_WINDOW_SEC else 0
