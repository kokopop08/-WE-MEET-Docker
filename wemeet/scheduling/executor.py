...
# 값 출처: sim_env.yaml scaling.max_task_attempts (오프라인 sim pretrain.py 과 공유).
MAX_TASK_ATTEMPTS = _ec.scaling()["max_task_attempts"]

# CSV 동시 쓰기 스레드 안전성 확보를 위한 전역 락 정의
...
def log_benchmark_metric(scheduler_mode, task_id, model_type, status, execution_time, cost, delay, virtual_budget):
    ...
    csv_file = paths.benchmark_csv(mode_str)
...
def run_task_on_worker(worker_id, worker_info, task, state, action):
    ...
    try:
        ...
        if FailureSimulator.check_oom(model_type, task_id, worker_info.get("node_type"), co_membound):
            _obslog.log_event(f"[Resource OOM] {task_id}: {worker_info.get('node_type')} 노드 메모리 경합 OOM 판정(동거 메모리바운드 {co_membound}개) -> 실패 처리 및 재큐잉")
            success = False
            execution_time = 0.0
            was_oom = True
        ...
        if is_map_reduce:
            ...
            _obslog.log_event(f"[Map-Merge] {task_id} 병렬 학습 분할 개시. 가용 IDLE 워커 수: {len(available_idle_workers)}")
            ...
            def execute_map_subtask(sub_idx, w_id, w_info):
                ...
                if os.path.exists(final_pt) and os.path.getsize(final_pt) > 0:
                    _obslog.log_event(f"[Map Skip] 서브맵 {sub_task_id} 이미 완료됨 -> 학습 Skip-Execution 처리.")
                ...
                if last_epoch > 0 and last_epoch < sub_ep:
                    ...
                    _obslog.log_event(f"[장애 복구] 서브맵 {sub_task_id} 중단 감지 -> {last_epoch} Epoch 가중치를 기반으로 이어서 학습 복구(남은 {actual_ep} Epochs) 시작.")

                sub_ip = w_info['ip']
                sub_address = f"[{sub_ip}]:{w_info['port']}" if ":" in sub_ip else f"{sub_ip}:{w_info['port']}"
                
                _obslog.log_event(f"[Map Task] 서브맵 할당: {sub_task_id} ({model_type}, {actual_ep} Ep) -> 워커 {w_id}")
                ...
                try:
                            ...
                            if stat.status in ["SUCCESS", "COMPLETED"]:
                                ...
                                if stat.logs:
                                    for line in stat.logs.splitlines():
                                        if line.strip():
                                            _obslog.log_event(f"[{w_id}] {line.strip()}")
                    ...
                    _obslog.log_event(f"[Map Task 에러] 서브맵 {sub_task_id} (워커 {w_id}) 실패: {ex}")
                    sub_success = False
                    with gcs_state.registry_lock:
                        if sub_task_id in gcs_state.task_lineage:
                            gcs_state.task_lineage[sub_task_id]["status"] = "FAILED"
            ...
            for attempt in range(max_retries):
                ...
                _obslog.log_event(f"[Map-Merge] {task_id} 시도 {attempt+1}/{max_retries} | 미완료 맵 서브태스크: {pending_indices}")
            ...
            if not all_maps_success:
                _obslog.log_event(f"[Map-Merge] 경고: 최대 재시도 한도 초과로 일부 맵 태스크가 최종 실패했습니다. 복구 복구 루프 재진입.")
                success = False
                ...
                if reduce_worker_id:
                    ...
                    _obslog.log_event(f"[Merge Task] 병합 가중치 생성 트리거: {reduce_task_id} -> 워커 {reduce_worker_id}")
                    try:
                                ...
                                if r_stat.status in ["SUCCESS", "COMPLETED"]:
                                    ...
                                    if r_stat.logs:
                                        for line in r_stat.logs.splitlines():
                                            if line.strip():
                                                _obslog.log_event(f"[{reduce_worker_id}] {line.strip()}")
                                    break
                                elif r_stat.status == "FAILED":
                                    reduce_success = False
                                    break
                    except Exception as rex:
                        _obslog.log_event(f"[Merge Task 에러] FedAvg 병합 실패: {rex}")
                        reduce_success = False
                    finally:
                        with gcs_state.registry_lock:
                    ...
                    execution_time = time.time() - start_time
                    if success:
                        _obslog.log_event(f"[Map-Merge] {task_id} 최종 Map-Merge FedAvg 병합 성공! (총 시간: {execution_time:.2f}초)")
                        ...
                        try:
                            ...
                            if merged_conclusion:
                                _obslog.log_event(f"[Conclusion Engine] {merged_conclusion}")
                            ...
                            _obslog.log_event(f"[Conclusion Engine 경고] 결론 도출 및 병합 중 오류: {e_conclusion}")
                        ...
                        for f_path in merge_files:
                            try:
                                if os.path.exists(f_path):
                                    os.remove(f_path)
                                    _obslog.log_event(f"[Merge Cleanup] 임시 맵 가중치 파일 소거 완료: {f_path}")
                            except Exception as cleanup_err:
                                _obslog.log_event(f"[Merge Cleanup 경고] 파일 {f_path} 소거 중 오류: {cleanup_err}")
                        ...
                        for cp_path in map_checkpoints:
                            try:
                                if os.path.exists(cp_path):
                                    os.remove(cp_path)
                                    _obslog.log_event(f"[Merge Cleanup] 임시 맵 체크포인트 파일 소거 완료: {cp_path}")
                            except Exception as cleanup_err:
                                _obslog.log_event(f"[Merge Cleanup 경고] 파일 {cp_path} 소거 중 오류: {cleanup_err}")
                        ...
                        try:
                            if os.path.exists(final_merged_path):
                                ...
                                _obslog.log_event(f"[Merge Cleanup] 최종 병합 가중치 파일 소거 완료: {final_merged_path}")
                        except Exception as cleanup_err:
                            _obslog.log_event(f"[Merge Cleanup 경고] 최종 병합 파일 {final_merged_path} 소거 중 오류: {cleanup_err}")
                    else:
                        _obslog.log_event(f"[Map-Merge] {task_id} Merge 병합 단계 실패.")
                        with gcs_state.registry_lock:
                            for sub_id in list(gcs_state.task_lineage.keys()):
                                lineage_info = gcs_state.task_lineage.get(sub_id)
                                if lineage_info and lineage_info.get("parent") == task_id:
                    ...
                    _obslog.log_event(f"[Merge Task 에러] 병합을 맡길 가용 워커가 존재하지 않습니다. 실패 처리.")
                    success = False
                    execution_time = time.time() - start_time
                    with gcs_state.registry_lock:
                        for sub_id in list(gcs_state.task_lineage.keys()):
            ...
            if result.status == "RUNNING":
                while True:
                    ...
                    if status_res.status in ["SUCCESS", "COMPLETED"]:
                        success = True
                        execution_time = time.time() - start_time
                        _obslog.log_event(f"[Scheduler Feedback] 작업 {task_id} 완료 성공! (실제 수행 시간: {execution_time:.2f}초)")
                        ...
                        try:
                            if status_res.logs:
                                for line in status_res.logs.splitlines():
                                    if any(tag in line for tag in ["[CNN Inference Done]", "[RNN Forecast Done]", "[LSTM Generation Done]"]):
                                        ...
                                        _obslog.log_event(f"[Conclusion Engine] {conclusion_text}")
                                        gcs_state.latest_conclusions.append({
                                            "task_id": task_id,
                                            "model_type": model_type,
                                            "timestamp": time.time(),
                            ...
                            _obslog.log_event(f"[Conclusion Engine 경고] 단일 결론 도출 오류: {e_conclusion}")
                        ...
                        try:
                            if os.path.exists(final_task_path):
                                ...
                                _obslog.log_event(f"[Task Cleanup] 최종 가중치 파일 소거 완료: {final_task_path}")
                        except Exception as cleanup_err:
                            _obslog.log_event(f"[Task Cleanup 경고] 파일 {final_task_path} 소거 중 오류: {cleanup_err}")
                        ...
                        for cp_path in task_checkpoints:
                            try:
                                if os.path.exists(cp_path):
                                    os.remove(cp_path)
                                    _obslog.log_event(f"[Task Cleanup] 체크포인트 파일 소거 완료: {cp_path}")
                            except Exception as cleanup_err:
                                _obslog.log_event(f"[Task Cleanup 경고] 파일 {cp_path} 소거 중 오류: {cleanup_err}")
                            
                        break
                    elif status_res.status == "FAILED":
                        success = False
                        execution_time = time.time() - start_time
                        _obslog.log_event(f"[Scheduler Feedback] 경고: 작업 {task_id} 연산 실패 리포트 수신.")
                        break
            else:
                _obslog.log_event(f"[Scheduler Feedback] 작업 개시 거부당함: {result.message}")
                
    except grpc.RpcError as e:
        _obslog.log_event(f"[Scheduler Feedback 에러] 워커 '{worker_id}' 실행 중 통신 크래시 감지: {e}")
        success = False
        execution_time = time.time() - task["enqueue_time"]
    except Exception as ex:
        import traceback
        tb = traceback.format_exc()
        _obslog.log_event(f"[Scheduler Error] Unexpected error running task {task_id} on {worker_id}: {ex}\n{tb}")
        success = False
        execution_time = time.time() - start_time
    finally:
        # 동적 cGroup 리소스 원복 복원
        ...
        # GCS 워커 노드 상태 복구 (OOM 발생 시 물리 노드 강제 삭제 및 Docker SDK 회수 진행)
        was_evicted = False
        with gcs_state.registry_lock:
            if worker_id in gcs_state.worker_registry:
                if was_oom:
                    # OOM-Kill 발생: 워커 레지스트리 즉각 제거 및 물리 컨테이너 강제 소거 실행
                    del gcs_state.worker_registry[worker_id]
                    import wemeet.cluster.manager as cluster_manager
                    threading.Thread(
                        target=cluster_manager.reclaim_corrupted_worker,
                        args=(worker_id,),
                        daemon=True
                    ).start()
                else:
                ...
                # 완료 시점에 워커가 레지스트리에서 사라진 상태에서 OOM이 아닌 경우 스팟 강제 회수(Eviction)로 판정
                if not was_oom:
                    was_evicted = True
        ...
        gcs_state.save_gcs_state()
        if was_oom:
            _obslog.log_event(f"[Scheduler Feedback] 작업 {task_id}: OOM 감지로 인한 실패 판정 -> 강화학습 OOM 페널티 부과")
        elif was_evicted:
            _obslog.log_event(f"[Scheduler Feedback] 작업 {task_id}: 스팟 회수(Eviction)로 인한 실패 판정 -> 강화학습 회수 페널티 부과")
        ...
        if gcs_state.SCHEDULER_MODE == "q_learning" and state is not None and action is not None:
            ...
            if gcs_state.Q_LEARNING_TRAINING_MODE:
                ...
                _obslog.log_event(f"[Q-Learning Update] State={state} | Action={action} | Reward={reward:.4f} | NextState={next_state} | Epsilon={agent.epsilon:.4f}")
                _obslog.log_event(f"[Q-Learning Update] 잔여 가상 예산: ${gcs_state.virtual_budget:.4f}달러")
            else:
                _obslog.log_event(f"[Q-Learning Inference] State={state} | Action={action} | Reward={reward:.4f} | NextState={next_state} (학습/저장 생략)")
        else:
            _obslog.log_event(f"[Resource Spend] [Mode: {gcs_state.SCHEDULER_MODE}] 비용 차감: ${task_cost:.4f} | 잔여 예산: ${gcs_state.virtual_budget:.4f}")
        ...
        if not success:
            ...
            if attempts >= MAX_TASK_ATTEMPTS:
                ...
                gcs_state.save_gcs_state()
                _obslog.log_event(f"[Dead Letter] 작업 {task_id} 최대 재시도({MAX_TASK_ATTEMPTS}회) 초과 -> 재배정 중단, 영구 실패 확정(큐에서 제외).")
                return
            ...
            if last_epoch > 0 and last_epoch < epochs:
                ...
                _obslog.log_event(f"[장애 복구] 작업 {task_id} 중단 감지 -> {last_epoch} Epoch 가중치를 기반으로 이어서 학습 복구(남은 {remaining_epochs} Epochs) 대기 큐 재할당. (재시도 {attempts}/{MAX_TASK_ATTEMPTS})")
                task["dataset_path"] = checkpoint_file
                task["epochs"] = remaining_epochs
            else:
                _obslog.log_event(f"[장애 복구] 작업 {task_id} 장애 유실 감지 -> 복구를 위해 대기 큐 재할당 (처음부터 재학습). (재시도 {attempts}/{MAX_TASK_ATTEMPTS})")
...
def log_online_training(state, action, reward, next_state, epsilon):
    ...
    from wemeet.config import paths
    csv_path = paths.online_training_csv()
