...
from wemeet.scheduling.qlearning_step import run_qlearning_scheduler_step
# Q-Learning 에이전트 및 연산 구동 공통 헬퍼 임포트 (task_executor에서 통합 로드)
from wemeet.scheduling.executor import agent, run_task_on_worker
...
def check_and_cleanup_dead_workers():
    ...
    with gcs_state.registry_lock:
        ...
        for wid in dead_workers:
            # 로그 기록
            _obslog.log_event(f"[Scheduler GCS] [DEAD 노드 감지] {wid} 노드가 오프라인 처리되었습니다.")
            # 레지스트리에서 삭제
            ...
            for sub_task_id, lineage_info in list(gcs_state.task_lineage.items()):
                if lineage_info["worker_id"] == wid and lineage_info["status"] == "RUNNING":
                    ...
                    if last_epoch > 0 and last_epoch < epochs:
                        ...
                        _obslog.log_event(f"[장애 복구] 유실된 subtask {sub_task_id} 중단 감지 -> {last_epoch} Epoch 가중치를 기반으로 이어서 학습 복구(남은 {recovered_epochs} Epochs) 대기 큐 재할당.")
                    else:
                        _obslog.log_event(f"[장애 복구] 유실된 subtask {sub_task_id} 장애 유실 감지 -> 복구를 위해 대기 큐 재할당 (처음부터 재학습).")
    ...
    if recovered_tasks:
        with gcs_state.queue_lock:
            for task in recovered_tasks:
                ...
                _obslog.log_event(f"[Lineage Recovery] !!! Cascaded Recovery 작동 !!! DEAD 워커에서 유실된 subtask '{task['task_id']}'를 대기열 최하단으로 복구했습니다!")
        gcs_state.save_gcs_state()
            
        # Auto Scale-out 연동: 유실된 노드를 대체하기 위해 스팟 노드 증설 요청
        _obslog.log_event(f"[Lineage Recovery] 노드 이탈로 인한 대체 자원 Scale-Out 요청 트리거")
        ...
        if wid.startswith("worker-2-") or wid.startswith("worker-3-"):
            ...
            try:
                if gcs_state.DOCKER_CLIENT is not None:
                    ...
                    _obslog.log_event(f"[Docker SDK] DEAD 컨테이너 회수 시작: {container_ref}")
                    container.stop(timeout=2)
                    container.remove()
                    _obslog.log_event(f"[Docker SDK] DEAD 컨테이너 회수 성공: {container_ref}")
            except Exception as e:
                _obslog.log_event(f"[Docker SDK 경고] DEAD 컨테이너 {container_ref} 회수 실패: {e}")

def generate_mock_tasks():
    """시뮬레이터 부하 검증을 위해 주기적으로 랜덤 가상 태스크를 생성하여 큐에 적재합니다."""
    # 생성 파라미터의 유일 진실: sim_env.yaml workload (env_config).
    wl = _ec.workload()
    model_types = wl["model_types"]

    # 가상 태스크 생성 빈도 (쉬는 시간 단축)
    # burst_prob 확률로 '태스크 폭풍(Burst)' 발생: burst_min~burst_max개가 한번에 유입
    # normal_prob 확률로 단일 태스크 유입
    is_burst = random.random() < wl["burst_prob"]
    is_normal = not is_burst and (random.random() < wl["normal_prob"])

    if is_burst:
        num_new_tasks = random.randint(wl["burst_min"], wl["burst_max"])
        _obslog.log_event(f"⚡ [Burst Traffic Alert] 태스크 폭발 유입 발생! (신규: {num_new_tasks}개)")
    elif is_normal:
        num_new_tasks = 1
    else:
        num_new_tasks = 0
    ...
    if num_new_tasks > 0:
        with gcs_state.queue_lock:
            if len(gcs_state.task_queue) < wl["queue_cap"]:  # 버스트 수용을 위한 최대 큐 크기
                for _ in range(num_new_tasks):
                    ...
                    epochs = random.randint(wl["epochs"]["min"], wl["epochs"]["max"])
                    timeout = random.randint(wl["timeout"]["min"], wl["timeout"]["max"])
                    ...
                    _obslog.log_event(f"[Task 유입] {task_id} ({model}, {epochs} Epochs) 큐 적재 완료. (마감기한: {timeout}초 후)")
        gcs_state.save_gcs_state()
...
def scheduler_loop():
    ...
    """
    _obslog.log_event("[Scheduler] Q-Learning 비용/SLA 인지형 의사결정 엔진 가동 성공.")
    
    model_types = ["CNN", "RNN", "LSTM"]
    
    import wemeet.cluster.resource_guard as resource_guard
    recommended_scale = resource_guard.get_recommended_max_spot_scale()
    MAX_SPOT_SCALE = max(_ec.scaling()["max_spot_scale_floor"], recommended_scale)
    _obslog.log_event(f"[Scheduler] 호스트 물리 RAM 감지 기반 MAX_SPOT_SCALE 설정 완료: {MAX_SPOT_SCALE}대")
    ...
    while True:
        ...
        if gcs_state.virtual_budget <= 0.0:
            if not budget_exhausted:
                ...
                _obslog.log_event(f"[예산 소진] 잔여 예산이 0 이하(${gcs_state.virtual_budget:.4f})로 파산. 신규 태스크 유입 중단 -> 진행 중 작업만 마무리 후 종료합니다.")
